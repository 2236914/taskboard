...
function writePaused(v: PausedSnapshot | null) {
  try {
    if (v) {
      localStorage.setItem(PAUSE_KEY, JSON.stringify(v));
    } else {
...
export function useActiveTimer() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel(
        `time-entries-active-${user.id}-${Math.random().toString(36).slice(2)}`,
      )
      .on(
        "postgres_changes",
        {
  ...
  const startInternal = useCallback(
    async (
      ...
      if (!opts?.silent) {
        toast.success(
          opts?.keepPrevious
            ? "New session started (previous kept running)"
            : "Clocked in",
        );
      }
  ...
  const resume = useCallback(async () => {
    ...
    const r = await startInternal(
      { tag_id: snap.tag_id, task_id: snap.task_id, note: snap.note },
      { silent: true },
    );
...
export function useTaskTime(taskId: string | null | undefined) {
  ...
  useEffect(() => {
    refreshRef2.current = refresh;
  }, [refresh]);
  useEffect(() => {
    if (!user || !taskId) return;
    const ch = supabase
      .channel(
        `time-entries-task-${taskId}-${Math.random().toString(36).slice(2)}`,
      )
      .on(
        "postgres_changes",
        {
  ...
  const updateEntry = async (
    ...
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data)
      setEntries((p) => p.map((e) => (e.id === id ? (data as TimeEntry) : e)));
  };
...
export function useAllTimeEntries() {
  ...
  useEffect(() => {
    refreshRef3.current = refresh;
  }, [refresh]);
  useEffect(() => {
    if (!user) return;
    const ch = supabase
      .channel(
        `time-entries-all-${user.id}-${Math.random().toString(36).slice(2)}`,
      )
      .on(
        "postgres_changes",
        {
  ...
  const createManualEntry = useCallback(
    async (input: {
      ...
      if (data) {
        setEntries((p) => [data as TimeEntry, ...p]);
  ...
  const updateEntry = useCallback(
    async (
      ...
      if (data) {
        setEntries((p) =>
          p.map((e) => (e.id === id ? (data as TimeEntry) : e)),
        );
