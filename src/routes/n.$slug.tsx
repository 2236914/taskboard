...
import { getPublicNote } from "@/server-functions/notes";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";

function NoteErrorComponent({
  error,
  reset,
}: {
  error: Error;
...
export const Route = createFileRoute("/n/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    const desc =
      (loaderData?.note?.content ?? "").slice(0, 160) ||
      "A note shared via Taskboard.";
    return {
      meta: [
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b">
        <div className="max-w-3xl mx-auto px-4 md:px-6 h-12 flex items-center gap-2">
          <div className="size-6 rounded-md bg-primary text-primary-foreground grid place-items-center font-semibold text-xs">
            T
          </div>
          <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
            Taskboard · shared note
          </span>
          <Button
            asChild
            size="sm"
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
              ...
              {note.tag && (
                <Badge
                  variant="outline"
                  className="font-mono text-[10px] gap-1 font-normal"
                >
                  <span
                ...
                {images.map((img) => (
                  ...
                  >
                    <img
                      ...
                      loading="lazy"
                    />
                    <div className="px-2 py-1.5 text-[10px] font-mono text-muted-foreground truncate">
                      {img.file_name}
                    </div>
                  </a>
                ))}
