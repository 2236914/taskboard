    ...
    return (
      <div className="rounded-lg border border-dashed bg-muted/30 py-8 px-4 text-center">
        ...
        <div className="text-[11px] text-muted-foreground">
          Create a task to see your status mix appear here.
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      <div className="h-3 w-full rounded-full overflow-hidden bg-muted flex">
        {todo > 0 && (
          <div
            title={`To do · ${todo}`}
            style={{ width: `${pct(todo)}%`, background: STATUS_COLOR.todo }}
          />
        )}
        {prog > 0 && (
          <div
            title={`In progress · ${prog}`}
            style={{
              width: `${pct(prog)}%`,
        ...
        {done > 0 && (
          <div
            title={`Done · ${done}`}
            style={{ width: `${pct(done)}%`, background: STATUS_COLOR.done }}
          />
        )}
      </div>
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] font-mono text-muted-foreground">
        ...
        <span className="ml-auto text-muted-foreground">
          ...
          <span className="text-foreground">
            {total ? Math.round((done / total) * 100) : 0}% done
          </span>
        </span>
      </div>
    </div>
...
export function TimeByTagBars({
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-dashed bg-muted/30 py-6 px-4 text-center text-xs text-muted-foreground">
        No time tracked in this range. Open a task and click{" "}
        <span className="text-foreground font-medium">Clock in</span> to start.
      </div>
    );
  }
  return (
          ...
          <div key={tag.id} className="space-y-1">
            ...
            <div className="h-2 rounded-full overflow-hidden bg-muted">
              <div
                ...
                style={{ width: `${w}%`, background: tag.color }}
              />
            </div>
          </div>
        );
