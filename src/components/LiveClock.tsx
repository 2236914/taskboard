...
type Loc = { city?: string; region?: string; country?: string; tz: string } | null;

function detectTz(): string {
  try { return Intl.DateTimeFormat().resolvedOptions().timeZone; } catch { return "UTC"; }
}

async function reverseGeocode(lat: number, lon: number): Promise<Partial<Loc> | null> {
  try {
...
export function LiveClock({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (cached) {
      try { setLoc(JSON.parse(cached)); return; } catch {}
    }
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        setLoc(next);
        try { sessionStorage.setItem("taskboard:loc", JSON.stringify(next)); } catch {}
      },
      () => { /* keep tz-only */ },
  ...
  return (
    <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
      <span className="text-2xl md:text-3xl font-semibold tabular-nums tracking-tight">{time}</span>
      ...
      <LocationPopover>
        <span className="ml-auto inline-flex items-center gap-1.5 text-[11px] font-mono text-muted-foreground">
          ...
          {locationLabel && <span className="text-[9px] uppercase tracking-widest text-primary ml-1">manual</span>}
        </span>
      </LocationPopover>
    </div>
  );
