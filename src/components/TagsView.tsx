...
export function TagsView() {
  const { tags, tasks, addTag, updateTag, deleteTag, mergeTag } =
    useTaskboard();
  ...
  const stats = useMemo(() => {
    ...
    tags.forEach((t) => {
      m[t.id] = { tasks: 0, notes: 0 };
    });
    tasks.forEach((t) => {
      if (t.tag_id && m[t.tag_id]) m[t.tag_id].tasks++;
    });
    notes.forEach((n) => {
      if (n.tag_id && m[n.tag_id]) m[n.tag_id].notes++;
    });
    return m;
  }, [tags, tasks, notes]);
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Categories with optional sub-tags. Each tag tracks how many tasks
            and notes use it.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2 items-center">
            ...
            <NewTagDialog
              onAdd={addTag}
              parents={tags.filter((t) => !t.parent_id)}
            />
          </div>
                ...
                return (
                  ...
                  >
                    <TagRow
                      ...
                      onSave={(patch) => {
                        updateTag(parent.id, patch);
                        setEditing(null);
                      }}
                      onDelete={() => {
                        if (
                          confirm(
                            `Delete "${parent.name}"${subs.length ? ` and ${subs.length} sub-tag(s)` : ""}? Items keep their data but lose this tag.`,
                          )
                        ) {
                          deleteTag(parent.id);
                        }
                      }}
                      mergeOptions={tags.filter(
                        (t) => t.id !== parent.id && !t.parent_id,
                      )}
                          ...
                          return (
                            <div key={sub.id} className="pl-6">
                              <TagRow
                                ...
                                onCancelEdit={() => setEditing(null)}
                                onSave={(patch) => {
                                  updateTag(sub.id, patch);
                                  setEditing(null);
                                }}
                                onDelete={() => {
                                  if (confirm(`Delete sub-tag "${sub.name}"?`))
                                    deleteTag(sub.id);
                                }}
                                mergeOptions={tags.filter(
                                  (t) => t.id !== sub.id,
  ...
  return (
    <div className="flex items-center gap-3 p-3 hover:bg-muted/40 transition group">
      ...
      <div className="flex gap-1.5">
        <Badge
          variant="outline"
          className="text-[10px] font-mono font-normal h-5"
        >
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            All {stats.tasks + stats.notes} item
            {stats.tasks + stats.notes === 1 ? "" : "s"} using "{tag.name}" will
            be reassigned to the chosen tag, then "{tag.name}" will be deleted.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="py-2">
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Optionally place it under a parent.
          </DialogDescription>
        </DialogHeader>
